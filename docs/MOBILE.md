# 手机访问 AgentQuad（Tailscale 私网模式）

> **目标**：在外面用手机打开 AgentQuad，看待办 / 继续 AI 会话 / 查统计。**不暴露到公网**。
>
...
### 4. 启动 AgentQuad 并开放 Tailscale 网口

默认 AgentQuad 只监听 `127.0.0.1`（安全第一）。要让 Tailscale 网络访问，必须显式放开：

```bash
# 方法 A（一次性）
agentquad start --expose
...
```
agentquad listening on http://all-interfaces:5677  (port 5677)

⚠️  SECURITY: agentquad exposes a shell + AI terminal. Reachable URLs:
...
- **方案 1（最简单）**：平时不挂 Tailscale，要用 AgentQuad 时在通知栏一键打开。
...
这是正常的，AgentQuad 跑在你的 Mac 上。Mac 醒着、AgentQuad 进程还活着、Tailscale 正常，手机才能访问。

**小技巧**：系统设置 → 节能 → **禁用"插电源时进入睡眠"**，可以让关盖的 Mac 连接电源时保持 AgentQuad 运行。
...
会给你一个永久的 `https://<name>.ts.net` 公网 URL。**这时对全公网开放**，强烈建议先在 AgentQuad 端加 token 鉴权（暂未实现；如果你需要可以提 issue）。
...
### Q: 访问 AgentQuad 报 "ERR_CONNECTION_REFUSED"

检查清单：

1. Mac 上 `agentquad status` 显示 running；
...
```bash
agentquad config set host 127.0.0.1
agentquad stop
agentquad start
```

或者临时用：`agentquad start --host 127.0.0.1`。
