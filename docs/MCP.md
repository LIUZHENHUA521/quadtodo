...
AgentQuad 内置了一个 [MCP](https://modelcontextprotocol.io) Streamable HTTP 服务端，挂在本机 `/mcp` 路径上。**任何支持 MCP 的客户端（Claude Code、Claude Desktop、Inspector 等）都可以连上，把 AgentQuad 当做一个"待办知识库"来读写**。
...
**典型玩法**：在任意 Claude Code session 里跟它说："帮我清理 AgentQuad 里重复的待办" → Claude 会调 `search` / `list_todos` 找出候选 → 把合并建议贴给你 → 你点头 → 它调 `merge_todos(confirm:true)` 真执行，自动在 `~/.agentquad/mcp-audit.log` 留痕。
...
如果你在别的设备上用 Claude Code 远程连 Mac 上的 AgentQuad，把 URL 里的 `127.0.0.1` 换成 Mac 的 Tailscale 名字或 IP（`agentquad mcp install --host <mac-name>`）。
...
✓ http://127.0.0.1:5677/mcp/health
  {"ok":true,"server":"agentquad",...}
```

在 Claude Code 里输入 `/mcp`，应该看到 `agentquad` 已连接 + 列出 17 个工具。
...
| `search_transcripts` | 在 `~/.agentquad/logs/*.log` 里做逐行扫描。支持 todoId / 时间范围过滤。默认上限 30 条命中 |
...
默认 `confirm:false`，返回预览 JSON + `howToConfirm` 指导；第二次带 `confirm:true` 才真执行，并写 `~/.agentquad/mcp-audit.log`。
...
同时 `~/.agentquad/mcp-audit.log` 追加一行 NDJSON：
...
⌘K（Mac）/ Ctrl+K（Windows）在 AgentQuad 网页端任意页面打开命令面板。和 MCP 共用同一个 `/api/search` 端点，所以搜索行为完全一致。
...
- 审计日志 `~/.agentquad/mcp-audit.log` 记录所有破坏性操作，失败也记。可以 `tail -f` 观察。

## 故障排查

| 症状 | 排查步骤 |
|---|---|
| Claude Code 里 `/mcp` 看不到 AgentQuad | 1. `agentquad mcp status` 确认服务健康；2. 检查 `~/.claude/settings.json` 里确实有 agentquad 条目；3. 重启 Claude Code |
| `status` 说 unreachable | AgentQuad 没跑或 port 不对。`npm start` 或改 `host`/`port` |
| 搜索结果为空但我知道有匹配 | 看 AgentQuad 启动日志有没有 `[search] fts ready`。如果上次异常退出过，加一行 `agentquad config set rebuild_fts true` 强制重建（触发下次启动时全量 rebuild） |
| `merge_todos` 报 `source_not_found` | 检查 sourceIds 是否正确。id 是 UUID 格式，区分大小写 |

## 开发笔记

如果你要给 AgentQuad 加新 MCP 工具：
